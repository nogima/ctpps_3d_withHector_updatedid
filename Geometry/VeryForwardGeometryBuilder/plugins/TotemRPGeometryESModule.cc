...
//    if ((! pD->name().name().compare(DDD_TOTEM_RP_DETECTOR_NAME))  || (! pD->name().name().compare("RPixWafer")))
    if ( pD->name().name().compare( DDD_TOTEM_RP_DETECTOR_NAME) == 0
	 ...
	 or pD->name().name().compare("RPixWafer")==0 )
    {
  ...
  if (alignments.isValid())
  {
    if (verbosity)
      std::cout 
        ...
        << alignments->GetSensorMap().size() << " sensor alignments applied."<< endl;
  } else {
    if (verbosity)
      std::cout 
        << ">> TotemRPGeometryESModule::produceRealGD > Real geometry: No alignments applied."<<endl;
  }
...
std::unique_ptr<DetGeomDesc> TotemRPGeometryESModule::produceMisalignedGD(const VeryForwardMisalignedGeometryRecord &iRecord)
{
  ...
  if (alignments.isValid())
  {
    ...
    if (verbosity)
      std::cout << ">> TotemRPGeometryESModule::produceMisalignedGD > Misaligned geometry: No alignments applied." << endl;
  }
