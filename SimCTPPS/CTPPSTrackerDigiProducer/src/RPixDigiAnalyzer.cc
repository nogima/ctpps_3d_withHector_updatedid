...
#include "SimCTPPS/CTPPSTrackerDigiProducer/interface/RPixDigiAnalyzer.h"
...
RPixDigiAnalyzer:: RPixDigiAnalyzer(const ParameterSet& pset) : theRPixDetTopology_(pset)
{

  label = pset.getUntrackedParameter<string>("label");  
  verbosity_ = pset.getParameter<int> ("Verbosity");
#ifdef USE_MIDDLE_OF_PIXEL
  file = new TFile("RPixDigiPlots2.root","RECREATE");
...
#else
  file = new TFile("RPixDigiPlots.root","RECREATE");
...
//  hAllHits = new TH2D("AllHits","All Hits",100,-5,5,100,-11,11);
  ...
  psim_token = consumes<PSimHitContainer>( edm::InputTag("g4SimHits","CTPPSTrackerHits") );
// psim_token = consumes<CrossingFrame<PSimHit>>(edm::InputTag("mix"));
  pixel_token = consumes<edm::DetSetVector<RPixDigi>>( edm::InputTag(label) ); //label=RPixDetDigitizer???

 
}

RPixDigiAnalyzer::~RPixDigiAnalyzer(){
}


void RPixDigiAnalyzer::beginJob(){
  found_corresponding_digi_count=0;
  for(int a=0; a<3; a++)  cumulative_cluster_size[a]=0;
}
void RPixDigiAnalyzer::endJob(){
...
void  RPixDigiAnalyzer::analyze(const Event & event, const EventSetup& eventSetup){
...
*/
//	event.getByLabel("g4SimHits", "CTPPSTrackerHits", simHits);


  edm::Handle< edm::DetSetVector<RPixDigi> > RPixDigis;
  event.getByToken(pixel_token, RPixDigis);
...
// Iterate on detector units
  edm::DetSetVector<RPixDigi>::const_iterator DSViter = RPixDigis->begin();

   for( ; DSViter != RPixDigis->end(); DSViter++) {
    ++numberOfDetUnits;
...
// looping over digis in a unit id
    edm::DetSet<RPixDigi>::const_iterator begin = (*DSViter).begin();
    edm::DetSet<RPixDigi>::const_iterator end = (*DSViter).end();

    if(verbosity_>2){
      std::cout << "FF  "<< DSViter->detId() << std::endl;
           for( edm::DetSet<RPixDigi>::const_iterator di = begin; di != end; di++){
...
DEFINE_FWK_MODULE(RPixDigiAnalyzer);
