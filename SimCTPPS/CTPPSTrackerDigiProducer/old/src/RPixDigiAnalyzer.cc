...
#define SELECTED_UNITID 2198863872
...
// psim_token = consumes<CrossingFrame<PSimHit>>(edm::InputTag("mix"));
  pixel_token = consumes<edm::DetSetVector<PixelDigi>>( edm::InputTag(label) ); //label=RPixDetDigitizer???
...
//	event.getByLabel("g4SimHits", "CTPPSTrackerHits", simHits);
  ...
  edm::Handle< edm::DetSetVector<PixelDigi> > RPixDigis;
...
// Iterate on detector units
  edm::DetSetVector<PixelDigi>::const_iterator DSViter = RPixDigis->begin();
...
// looping over digis in a unit id
  ...
  //   if(DSViter->detId()/1e6 >2198.8 && DSViter->detId()/1e6 <2198.9) {    // looping only on one plane
    if(DSViter->detId() == SELECTED_UNITID && found_hit_inside_selected_pixel
      ...
      for( edm::DetSet<PixelDigi>::const_iterator di = begin; di != end; di++){
