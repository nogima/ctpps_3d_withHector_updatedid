...
#include "SimCTPPS/CTPPSTrackerDigiProducer/interface/CTPPSTrackerDigiProducer.h"
...
CTPPSTrackerDigiProducer::CTPPSTrackerDigiProducer(const edm::ParameterSet& conf) :
	conf_(conf) {
...
//	produces<edm::DetSetVector<RPDetTrigger> > ();
	...
	tokenCrossingFrameCTPPSTracker = consumes<CrossingFrame<PSimHit>>(edm::InputTag("mix","g4SimHitsCTPPSTrackerHits"));
...
CTPPSTrackerDigiProducer::~CTPPSTrackerDigiProducer() {
...
void CTPPSTrackerDigiProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
	using namespace edm;
...
//	iEvent.getByToken(tokenCrossingFrameTotemRP, cf);
	iEvent.getByToken(tokenCrossingFrameCTPPSTracker, cf);
//	iEvent.getByLabel("mix", "g4SimHitsCTPPSTrackerHits", cf);
	...
	for (simhit_map_iterator it = SimHitMap.begin(); it != SimHitMap.end(); ++it) { // secondo me qua il loop viene fatto sui rivelatori
		...
		edm::DetSet<PixelDigi> digi_collector(it->first);
		...
		std::vector<PixelDigi>::iterator pixelIterator = digi_collector.data.begin();
...
//		}
	...
	std::auto_ptr<edm::DetSetVector<PixelDigi> > digi_output(
								 new edm::DetSetVector<PixelDigi>(theDigiVector));    // produco la collezione di output
...
void CTPPSTrackerDigiProducer::beginRun(edm::Run&, edm::EventSetup const& es){
...
void CTPPSTrackerDigiProducer::endJob() {
}

DEFINE_FWK_MODULE( CTPPSTrackerDigiProducer);
